...
from fuc_tools import top_k_top_p_generate
from fuc_tools import top_k_hierarchy_generate
...
class GPT2PositionalEncoding(nn.Module):
    def __init__(self, config):
        ...
        self.sqrt_dim = torch.tensor([1])
        if config.embedding_sqrt_mul:
            self.sqrt_dim = torch.sqrt(torch.tensor(config.hidden_dim))

    def forward(self, input_ids):
        len_idx = input_ids.shape[1]
        encoded_ids = self.embedding(input_ids) * self.sqrt_dim.to(input_ids.device)
        encoded_ids = encoded_ids + self.pos_code[:, :len_idx, :].to(input_ids.device)
        return self.dropout(encoded_ids)
...
class GPT2(nn.Module):
    ...
    def save_to_trained(self, file):
        if not os.path.exists(file):
            os.mkdir(file)
        torch.save(self.state_dict(), file + '/' + 'model.pt')

    def load_from_file(self, file):
        model_state_dic = torch.load(file)
        self.load_state_dict(model_state_dic)

    def load_from_trained(self, file):
        model_state_dic = torch.load(file + '/' + 'model.pt')
        self.load_state_dict(model_state_dic)
...
class GPT2SeqGeneratorConfig:
    def __init__(
        ...
        self.device = device
        self.for_rm_train = for_rm_train
...
class GPT2SeqGenerator:
    ...
    def generate(self, input_sentence, only_id=False):
        # input: string
        tokenized_sentence = self.tokenizer.tokenize(input_sentence)
        token_ids, _ = self.tokenizer.encode_text(tokenized_sentence)
        prompts = token_ids[0]
        input_ids = torch.tensor(token_ids, dtype=torch.long, device=self.config.device)
        end_mark_id = self.tokenizer.token_to_id['[EOS]']
        ans, attention_weight_seq = [], []

        for i in range(self.config.output_max_len):
            logits = self.model(input_ids)['logits']
            token_predicted = None
            last_logits = logits[:, -1, :].squeeze(dim=0)
            if self.config.generate_pattern == 'top_k' or self.config.generate_pattern == 'top_p':
                token_predicted = top_k_top_p_generate(last_logits, top_k=self.config.top_k, top_p=self.config.top_p)
            elif self.config.generate_pattern == 'top_k_hierarchy':
                token_predicted = top_k_hierarchy_generate(last_logits,
                                                           top_k=self.config.top_k,
                                                           hierarchy=self.config.hierarchy
                                                           )
            input_ids = torch.cat((input_ids, token_predicted.unsqueeze(dim=-1)), dim=-1)

            if self.config.save_attention_weights:
                attention_weight_seq.append(self.model.decoder.attention_weights)

            if token_predicted.item() == end_mark_id:
                break
            ans.append(token_predicted.item())

        if self.config.for_rm_train:
            return prompts, ans

        if only_id:
            return ans

        return self.tokenizer.decode(ans), attention_weight_seq
...
class GPT2Config:
    def __init__(
        ...
        self.q_size = hidden_dim
        self.k_size = hidden_dim
        self.v_size = hidden_dim
    ...
    def save_to_trained(self, model_pth):
        if not os.path.exists(model_pth):
            os.mkdir(model_pth)
        pth = model_pth + '/' + 'config.json'
    ...
    @classmethod
    def load_from_trained(cls, file):
        with open(file + '/' + 'config.json', 'r') as f:
            config_dic = json.load(f)
            return cls(**config_dic)
