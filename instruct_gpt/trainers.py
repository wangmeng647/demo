...
class TransformerTrainer:
    ...
    def train(self, train_iter):
        ...
        for i in range(self.config.epochs):
            ...
            print(f'{end - start} sec per epoch')
        if self.config.save:
...
class GPT2TrainerConfig:
    def __init__(
        ...
        self.epoch_num_to_save = epoch_num_to_save
        self.warmup = warmup
...
class GPT2Trainer:
    def __init__(self, config, model, optimizer=None):
        ...
        self.optimizer = optimizer
        if not optimizer:
            raise Exception('please input optimizer')

    def apply_init_weight(self, init_fn):
        self.model.apply(init_fn)

    def train(self, train_iter, model_pth='model'):
        ...
        if self.config.warmup:
            scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=self.config.warmup)

        if self.config.visdom_open:
            window = Visdom()
            window.line([0], [0], win='loss', opts=dict(title='loss'))

        if self.config.save or self.config.save_epoch:
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for batch in train_iter:
                ...
                optimizer.step()
                loss_accumulated += loss.detach().cpu()
                tokens_accumulated += input_mask[:, 1:].sum().detach().cpu()
            end = time.time()
            ...
            print(f'loss: {loss_accumulated / tokens_accumulated:.3f} per token, epoch: {epoch_i + 1}')
            print(f'speed_on_token: {tokens_accumulated / (end - start):.1f} tokens per sec on {self.config.device}')
            print(f'speed_on_epoch: {end - start} sec per epoch')

            if self.config.save_epoch:
                if (epoch_i + 1) % self.config.epoch_num_to_save == 0:
                    check_point = {'model': model.state_dict(),
                                   'optimizer': optimizer.state_dict(),
                                   'epoch': epoch_i + 1,
                                   'loss_per_token': loss_accumulated / tokens_accumulated}
                    torch.save(check_point, model_pth + '/' + f'check_point_epoch_{epoch_i + 1}.pt')
        ...
        if self.config.save:
            check_point = {'model': model.state_dict(),
                           ...
                           'loss_per_token': loss_accumulated / tokens_accumulated}
            torch.save(check_point, model_pth + '/' + f'check_point_final.pt')
...
class GPT2RMTrainer:
    ...
    def train(self, train_iter):
        ...
        for i in range(self.config.epochs):
            ...
            start = time.time()
            for batch in train_iter:
                batch_reward_table = torch.tensor([], device=self.config.device)

                # batch_ids sample like [[[1, 2, 3], [1, 2, 5]], [[4, 2, 3], [4, 2, 5]]]  batch_mask same with id
            ...
            print(f'loss: {loss_accumulated / batch_num_accumulated:.3f} per sentence, epoch: {i}')
            print(f'speed: {batch_num_accumulated / (end - start):.1f} sentences per sec on {self.config.device}')
        if self.config.save:
            torch.save(model.state_dict(), 'state_dic_transformer.pt')
...
class PPOTrainer:
    """
    train a model with ppo algorithm
    """
    ...
    def train(self, train_iter):
        ...
        config = self.config
        pg_model.to(device=self.config.device)
        refer_model.to(device=self.config.device)
        vf_model.to(device=self.config.device)
        for i in range(config.epochs):
            ...
            for batch in train_iter:
                ...
                for ppo_epoch in range(self.config.ppo_epochs):
                    ...
                    entropy_loss_accu += entropy_loss.detach()
            ...
            print(f'pg_loss: {ave_pg_loss:.3f} on epoch {i}')
            print(f'vf_loss: {ave_vf_loss:.3f} on epoch {i}')
            if self.config.entropy_loss:
                print(f'entropy_loss: {ave_entropy_loss:.3f} on epoch {i}')
            print(f'speed: {int(token_num_accu / (end - start + 1e-7)):.1f} tokens per sec on {self.config.device}')
    ...
    def prob_forward(model, batch_ids):
        """
        gather the next token prob, excluding the first token
        """
    ...
    def returns_and_advantages(self, prob_refer, prob_base, value_base, reward, batch_mask):
        """
        compute the advantages of the next token chosen from the end of prompt
